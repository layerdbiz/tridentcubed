<script lang="ts">
	...
	function getPhotoPageDescription(
		page: projectTypes.PreviewPageItemType,
	): string {
		if (page.section?.type !== 'photos') return '';
		if (page.section.description) return page.section.description;

		if (page.title === 'Introduction') {
			return reportSubtitle || 'Project overview and inspection context.';
		}

		if (page.title === 'Cargo Description') {
				...
				<div class="relative w-full px-1 pb-10 pt-0 md:px-0 md:pb-16 md:pt-2">
					<div class="w-full" style={`--preview-zoom: ${previewZoom || 1}; --preview-page-width: ${projectConstants.previewPageWidth}px; --preview-page-height: ${projectConstants.previewPageHeight}px`}>
						<div bind:this={previewPages} class="flex flex-col items-center gap-4 md:gap-12">
							{#each previewPageItems as page (page.id)}
								{#if page.kind === 'cover'}
								...
								{:else if page.kind === 'template'}
									<PreviewPage>
										<Text h2={page.title} class="mb-6 text-4xl" />
								...
								{:else if page.kind === 'time-log' && page.section?.type === 'time-log'}
									<PreviewPage>
										<Text h2={page.title} class="mb-4 text-4xl" />
										{#each page.section.days as day (day.id)}
								...
								{:else if page.kind === 'photo' && page.section?.type === 'photos'}
									<PreviewPage>
										<Text h2={page.title} class="mb-4 text-4xl" />
										...
										<div class={projectUtils.getPreviewPhotoGridClass(page.section)}>
											{#if page.section.photos.length}
												{#each page.section.photos as photo (photo.id)}
													<figure class={projectUtils.getPreviewPhotoCardClass(page.section, photo)}>
														<div class="grid place-items-center bg-neutral-50 p-3" style={`height: ${projectUtils.getPreviewPhotoFrameHeight(page.section, photo)}`}>
