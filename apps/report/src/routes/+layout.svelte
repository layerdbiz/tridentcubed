<script lang="ts">
	...
	// Fetch slides navigation data (prerender functions use .current)
	const chaptersData = getChapterNavigationData().current ?? [];
	const slidesData = getSlidesData().current ?? [];
...
<main class="grid h-svh max-h-svh w-full">
	<div>
