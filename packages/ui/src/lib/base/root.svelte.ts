...
export function getPlacementAliasMap(): Record<string, string> {
	const layoutProps = snippetLib.aliases?.layoutProps;
	if (!layoutProps) return {};
	if (!placementAliasMapCache) {
		placementAliasMapCache = createPlacementAliasMap(layoutProps);
	}
	return placementAliasMapCache;
}
...
export function resolveSnippet(
	...
	for (const candidate of candidates) {
		...
		if (snippetLib.hasRenderableValue(snippet)) {
			return {
				base,
				key,
				label: candidate.key,
...
export function createDebugItems(): ResolvedItem[] {
	return snippetLib.cellNames.map((base) => ({
		base,
		key: `debug-${base}`,
		label: base,
		className: "is-debug",
