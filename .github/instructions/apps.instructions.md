...
- For manual or native-element persistence in route code, import the shared `persist` singleton from `@layerd/ui`. Use `persist.sync(...)` for route-owned arrays or objects that should auto-load and auto-save through `getValue` and `setValue`, and use `persist.save`, `persist.load`, `persist.remove`, or `persist.clear` directly only when you need one-off imperative storage work. For Component-driven persistence, keep using the single `persist` prop instead of route-local helper props or controller factories.
